...
export async function getSiteConfig(): Promise<SiteConfig> {
  ...
  try {
    const [index, branding, contact, theme, variants] = await Promise.all([
      readFile(getConfigPath("sites", siteId, "index.json"), "utf-8").then(JSON.parse),
      readFile(getConfigPath("sites", siteId, "branding.json"), "utf-8").then(JSON.parse),
      readFile(getConfigPath("sites", siteId, "contact.json"), "utf-8").then(JSON.parse),
      readFile(getConfigPath("sites", siteId, "theme.json"), "utf-8").then(JSON.parse),
      readFile(getConfigPath("sites", siteId, "variants.json"), "utf-8").then(JSON.parse),
    ])
 ...
 */
export async function getServiceConfig(serviceSlug: string): Promise<ServicePageConfig> {
  const siteId = getSiteId()

  try {
    const content = await readFile(getConfigPath("sites", siteId, "services", `${serviceSlug}.json`), "utf-8")
...
export async function loadServiceConfig(siteId: string, serviceSlug: string): Promise<ServicePageConfig> {
 ...
 */
export async function getLocationConfig(locationSlug: string): Promise<ServicePageConfig> {
  const siteId = getSiteId()

  try {
    const content = await readFile(getConfigPath("sites", siteId, "locations", `${locationSlug}.json`), "utf-8")
    return JSON.parse(content)
  } catch (error) {
    console.error(`[v0] Failed to load location config ${locationSlug}:`, error)
    throw error
  }
}

export async function loadLocationConfig(siteId: string, locationSlug: string): Promise<ServicePageConfig> {
  // siteId is ignored since we use getSiteId() internally
...
export async function getPageConfig(pageName: string): Promise<PageConfig> {
  ...
  try {
    const content = await readFile(getConfigPath("sites", siteId, "pages", `${pageName}.json`), "utf-8")
    return JSON.parse(content)
  } catch (error) {
...
export async function getNavigationConfig() {
  ...
  try {
    const content = await readFile(getConfigPath("sites", siteId, "navigation.json"), "utf-8")
    return JSON.parse(content)
  } catch (error) {
...
export async function getFooterConfig(): Promise<FooterConfig> {
  ...
  try {
    const content = await readFile(getConfigPath("sites", siteId, "footer.json"), "utf-8")
    return JSON.parse(content)
  } catch (error) {
